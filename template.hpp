...
TreeNode* TreeFromLevelOrder(const vi& arr) {
    ...
    while (not q.empty()) {
        ...
        q.pop_front();
        if (idx < n) {
            at->left = (arr[idx] == -INF) ? nullptr : (new TreeNode(arr[idx]));
            ++idx;
            if (at->left) {
                q.push_back(at->left);
                at->left->parent = at;
        ...
        if (idx < n) {
            at->right = (arr[idx] == -INF) ? nullptr : (new TreeNode(arr[idx]));
            ++idx;
            if (at->right) {
...
void print_list(Node* head) {
    for (Node* curr = head; curr; curr = curr->right) cout << curr->value << ' ';
    cout << endl;
}

void print_list2(Node* head) {
    for (Node* curr = head; curr; curr = curr->next) cout << curr->value << ' ';
    cout << endl;
}
